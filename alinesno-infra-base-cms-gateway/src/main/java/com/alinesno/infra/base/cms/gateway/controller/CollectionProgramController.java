...
/**
 * 处理与CollectionProgramEntity相关的请求的Controller。
 * 继承自BaseController类并实现ICollectionProgramService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class CollectionProgramController extends BaseController<CollectionProgramEntity, ICollectionProgramService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(CollectionProgramController.class);

    @Autowired
    private ICollectionProgramService service;

    /**
     * 获取CollectionProgramEntity的DataTables数据。
     *
    ...
    @Override
    public ICollectionProgramService getFeign() {
        return this.service;
    }
}