...
/**
 * 处理与DatacallEntity相关的请求的Controller。
 * 继承自BaseController类并实现IDatacallService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class DatacallController extends BaseController<DatacallEntity, IDatacallService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(DatacallController.class);

    @Autowired
    private IDatacallService service;

    /**
     * 获取DatacallEntity的DataTables数据。
     *
    ...
    @Override
    public IDatacallService getFeign() {
        return this.service;
    }
}