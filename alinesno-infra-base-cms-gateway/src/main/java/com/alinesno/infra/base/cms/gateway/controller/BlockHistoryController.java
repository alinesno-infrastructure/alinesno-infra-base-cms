...
/**
 * 处理与区块历史记录相关的请求的Controller。
 * 继承自BaseController类并实现IBlockHistoryService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class BlockHistoryController extends BaseController<BlockHistoryEntity, IBlockHistoryService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(BlockHistoryController.class);

    @Autowired
    private IBlockHistoryService service;

    /**
     * 获取区块历史记录的DataTables数据。
     *
    ...
    @Override
    public IBlockHistoryService getFeign() {
        return this.service;
    }
}