...
/**
 * 处理与ContentCheckEntity相关的请求的Controller。
 * 继承自BaseController类并实现IContentCheckService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class ContentCheckController extends BaseController<ContentCheckEntity, IContentCheckService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(ContentCheckController.class);

    @Autowired
    private IContentCheckService service;

    /**
     * 获取ContentCheckEntity的DataTables数据。
     *
    ...
    @Override
    public IContentCheckService getFeign() {
        return this.service;
    }
}