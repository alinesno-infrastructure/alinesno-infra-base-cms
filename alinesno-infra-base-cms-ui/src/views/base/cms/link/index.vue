...
import { listLinkGroup, addLinkGroup, updateLinkGroup, delLinkGroup } from "@/api/base/cms/link/linkGroup";
...
export default {
  ...
  methods: {
    loadListData () {
      this.loading = true;
      listLinkGroup(this.queryParams).then(response => {
        this.linkGroupList = response.rows;
        this.loading = false;
      });
    },
    cancel () {
    ...
    submitForm () {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id) {
            updateLinkGroup(this.form).then(response => {
    ...
    handleDelete(row) {
      const rows = row.id ? [ row ] : this.selectedRows
      this.$modal.confirm(this.$t('Common.ConfirmDelete')).then(function() {
        return delLinkGroup(rows);
      }).then((response) => {
    ...
    handleGroupClick(row) {
      this.$router.push({
        path: "/Content/base/cms/link/list",
        query: {
          groupId: row.id
        }
      });
