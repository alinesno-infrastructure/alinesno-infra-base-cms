    ...
    <el-row>
      <el-col :span="24">
        <div class="grid-btn-bar bg-purple-white">
          <el-button plain type="success" size="default" icon="el-icon-edit" @click="handleSave">{{ $t("Common.Save") }}</el-button>
          <el-button plain type="primary" size="default" icon="el-icon-timer" @click="handleToPublish">{{ $t('CMS.ContentCore.ToPublish') }}</el-button>
          <el-button plain type="primary" size="default" icon="el-icon-s-promotion" @click="handlePublish">{{ $t('CMS.ContentCore.Publish') }}</el-button>
          <el-button plain type="primary" size="default" @click="handlePreview"><svg-icon icon-class="eye-open" class="mr5"></svg-icon>{{ $t('CMS.ContentCore.Preview') }}</el-button>
          <el-button plain type="warning" v-if="isLock" size="default" icon="el-icon-unlock" @click="handleChangeLockState">{{ $t('CMS.Content.Unlock') }}</el-button>
          <el-button plain type="primary" v-else size="default" icon="el-icon-lock" @click="handleChangeLockState">{{ $t('CMS.Content.Lock') }}</el-button>
          <el-button plain type="primary" size="default" icon="el-icon-share" @click="handleRelaContent">{{ $t('CMS.Content.RelaContent') }}</el-button>
          <el-button plain type="primary" size="default" icon="el-icon-files" @click="handlePushToBaidu">{{ $t('CMS.Content.PushToBaidu') }}</el-button>
          <el-button v-if="openEditorW" plain type="warning" size="default" icon="el-icon-close" @click="handleClose">{{ $t('Common.Close') }}</el-button>
          <el-button v-else plain type="warning" size="default" icon="el-icon-back" @click="handleGoBack">{{ $t('CMS.Content.BackToList') }}</el-button>
        </div>
      </el-col>
    </el-row>
...
<script>
import { getInitContentEditorData, addContent, saveContent, toPublishContent, publishContent, lockContent, unLockContent, moveContent } from "@/api/base/cms/content";
// import { getUEditorCSS } from "@/api/contentcore/article"
// import { pushToBaidu } from "@/api/seo/baidupush";
// import Sticky from '@/components/Sticky'
import UEditor from '@/views/components/UEditorPlus'
    ...
    handleGoBack() {
      ...
      if (this.isFormChanged()) {
        this.$confirm(this.$t('CMS.Content.CloseContentEditorTip'), this.$t('Common.SystemTip'), {
          ...
          const obj = { path: "/Content/base/cms/post/index" };
