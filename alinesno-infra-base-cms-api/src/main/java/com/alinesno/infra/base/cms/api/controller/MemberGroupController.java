...
/**
 * 处理与MemberGroupEntity相关的请求的Controller。
 * 继承自BaseController类并实现IMemberGroupService接口。
 * 
 * @author luoxiaodong
 * @version 1.0.0
 */
...
public class MemberGroupController extends BaseController<MemberGroupEntity, IMemberGroupService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(MemberGroupController.class);

    @Autowired
    private IMemberGroupService service;

    /**
     * 获取MemberGroupEntity的DataTables数据。
     *
    ...
    @Override
    public IMemberGroupService getFeign() {
        return this.service;
    }
}