...
/**
 * 处理与CommentTableEntity相关的请求的Controller。
 * 继承自BaseController类并实现ICommentTableService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class CommentTableController extends BaseController<CommentTableEntity, ICommentTableService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(CommentTableController.class);

    @Autowired
    private ICommentTableService service;

    /**
     * 获取CommentTableEntity的DataTables数据。
     *
    ...
    @Override
    public ICommentTableService getFeign() {
        return this.service;
    }
}